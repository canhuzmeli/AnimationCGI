...
bool ContinuousTimeCollisionHandler::detectParticleParticle(const TwoDScene &scene, const VectorXs &qs, const VectorXs &qe, int idx1, int idx2, Vector2s &n, double &time)
{
    ...
    double TSQUARED_COEF_P  = (-1*(dx2-dx1)).dot(dx2-dx1);
    double T_COEF_P = (-2*(x2-x1)).dot(dx2-dx1);
    double CONSTANT_COEF_P = pow((r1+r2),2) - ((x2-x1).dot(x2-x1));
    
    // Do not change the order of the polynomials here, or your program will fail the oracle
    std::vector<Polynomial> polynomials;
    
    position_polynomial.push_back(TSQUARED_COEF_P);   // position polynomial for particle-particle detection is quadratic
    position_polynomial.push_back(T_COEF_P);
    position_polynomial.push_back(CONSTANT_COEF_P);
    

  
    double T_COEF_V = (dx1-dx2).dot(dx2-dx1);
    double CONSTANT_COEF_V = (dx1-dx2).dot(x2-x1);
    
    velocity_polynomial.push_back(T_COEF_V);      // velocity polynomial for particle-particle detection is linear
    velocity_polynomial.push_back(CONSTANT_COEF_V);
    ...
    if(time >= 0 && time <= 1){
        ...
        //std::cout << time << std::endl;
        dx = time*dx;
        dx1 = dx.segment<2>(2*idx1);
        dx2 = dx.segment<2>(2*idx2);
        x1 = x1+dx1;
        x2 = x2+dx2;
        n = x2-x1;
        return true;
...
bool ContinuousTimeCollisionHandler::detectParticleHalfplane(const TwoDScene &scene, const VectorXs &qs, const VectorXs &qe, int vidx, int pidx, Vector2s &n, double &time)
{
    ...
    VectorXs x2 = scene.getHalfplane(pidx).first;
    ...
    double TSQUARED_COEF_P  = ((dx1).dot(-dx1));
    double T_COEF_P = 2*(((x2*np).dot(x1)) - (x1.dot(dx1)));
    double CONSTANT_COEF_P = pow((r1+r2),2) + 2*((x2*np).dot(x1)) - (x2*np).dot(x2*np) - x1.dot(x1)  ;
    ...
    double T_COEF_V =-1*(dx1.dot(dx1));
    double CONSTANT_COEF_V = dx1.dot(x1*np) - dx1.dot(x1);
