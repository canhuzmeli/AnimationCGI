...
{	
    ...
    VectorXs qe_temp = qe;
    VectorXs qs_temp = qs;
    VectorXs qdote_temp = qdote;
    std::vector<CollisionInfo> collisions = detectCollisions(scene,qs_temp,qe_temp);
    // Your code goes here!
    if(collisions.size()>0){        
        for(int i=0; i < m_maxiters; i++){
            applyImpulses(scene, collisions, qs_temp, qe_temp, qdote_temp, dt,qefinal, qdotefinal);
            qe_temp = qefinal;
            qdote_temp = qdotefinal;
            collisions = detectCollisions(scene,qs_temp,qe_temp);
            if(collisions.size()==0){
                return true;
            }
...
void HybridCollisionHandler::performFailsafe(const TwoDScene &scene, const VectorXs &qs, const ImpactZone &zone, double dt, VectorXs &qe, VectorXs &qdote)
{
    ...
    //
    VectorXs xscm;
    VectorXs sigmamixis(2);
    sigmamixis[0] = 0;
    sigmamixis[1] = 0;
    const VectorXs &M = scene.getM();
    double totalMass = 0;
    for(unsigned int i = 0; i , zone.vertices.size(); i++){        
        sigmamixis += (M[2*i] * scene.getV().segment<2>(2*vertices[i]));
        totalMass += M[2*i];
    }
    xscm = sigmamixis / totalMass;
...
void HybridCollisionHandler::applyGeometricCollisionHandling(const TwoDScene &scene, const VectorXs &qs, const VectorXs &qe, const VectorXs &qdote, double dt, VectorXs &qm, VectorXs &qdotm)
{
    ...
    ImpactZones zones;
    growImpactZones(scene, zones, collisions);

        
    while(true){
        for(unsigned int i=0; i < zones.size(); i++){
            performFailsafe(scene,qs,zones[i],dt,qm,qdotm);
        }
        
        collisions = detectCollisions(scene,qs,qm);
        ImpactZones zones_dot;
        growImpactZones(scene, zones_dot, collisions);
        for(unsigned int j=0; j < zones.size(); j++){
            zones_dot.push_back(zones[j]);
        }
        mergeAllZones(zones_dot);
        if(zonesEqual(zones_dot,zones)){
            break;
        }else{
            zones = zones_dot;
        }
